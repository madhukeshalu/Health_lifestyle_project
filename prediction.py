...
# --------------------------------------------------
# Absolute path to current file (prediction.py)
# --------------------------------------------------
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# models folder inside project
...
try:
    ...
    print("Expected location:", MODEL_DIR)
    print("Available files:", os.listdir(MODEL_DIR) if os.path.exists(MODEL_DIR) else "models folder missing")
    raise e
...
def generate_recommendations(bmi: float, wellness_score: float, risk_category: str, user_input: dict) -> dict:
    ...
    # Categorized recommendations for the UI
    ...
    for factor in risk_factors:
        suggestion = risk_factor_suggestions.get(factor)
        if not suggestion: continue
        
        if any(kw in factor for kw in ['Diet', 'Fast food', 'Water', 'Nutrition']):
            recommendations_dict["Nutrition & Diet"].append(suggestion)
        elif any(kw in factor for kw in ['Activity', 'Exercise', 'BMI', 'Overweight', 'Obesity']):
            recommendations_dict["Physical Activity"].append(suggestion)
        elif 'Sleep' in factor:
            recommendations_dict["Sleep Quality"].append(suggestion)
        elif any(kw in factor for kw in ['Stress', 'Wellness', 'Wellbeing']):
            recommendations_dict["Stress & Mental Health"].append(suggestion)
        elif any(kw in factor for kw in ['Smoking', 'Alcohol', 'Screen']):
            recommendations_dict["Lifestyle Habits"].append(suggestion)
        elif any(kw in factor for kw in ['Chronic', 'Family', 'Medical']):
            recommendations_dict["Medical & Monitoring"].append(suggestion)
    
    # Add general category-based advice if empty
    if not recommendations_dict["Nutrition & Diet"]:
        recommendations_dict["Nutrition & Diet"].append("Maintain a balanced diet rich in whole foods and fiber.")
    if not recommendations_dict["Physical Activity"]:
        recommendations_dict["Physical Activity"].append("Aim for at least 30 minutes of moderate activity most days.")
    
    # Add general recommendations based on risk category
    if risk_category == "High Risk":
        recommendations_dict["Medical & Monitoring"].append("Schedule a comprehensive health check-up with your doctor immediately.")
        recommendations_dict["Medical & Monitoring"].append("Consider specialist consultation for identified high-risk areas.")
    elif risk_category == "Medium Risk":
        recommendations_dict["Medical & Monitoring"].append("Monitor your key health metrics (BMI, BP) monthly.")
        recommendations_dict["Lifestyle Habits"].append("Focus on small, sustainable lifestyle improvements.")
    
    # Clean up empty categories
    recommendations_dict = {k: v for k, v in recommendations_dict.items() if v}
    
    # Create numbered string fallback for simple text display
    suggestions_list = []
    for cat, items in recommendations_dict.items():
        suggestions_list.extend(items)
    health_suggestion = "\n".join([f"{i+1}. {s}" for i, s in enumerate(suggestions_list[:10])])
